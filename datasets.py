...
class GroupDataset:
    def __init__(
            self, split, root, metadata, transform,
            subsample_what=None, duplicates=None):
        self.transform_ = transform
        with open(metadata, "r") as f:
            lines = f.readlines()

        self.i, self.x, self.y, self.g = [], [], [], []

        i = 0
        for line in lines:
            _, fname, yi, si, gi, _ = line.split(",")

            if (split == "tr" and si == "0") or\
               (split == "va" and si == "1") or\
               (split == "te" and si == "2"):
                self.x.append(os.path.join(root, fname))
                self.y.append(int(yi))
                self.g.append(int(gi))
                self.i.append(i)
                i += 1
    ...
    def count_groups(self):
        ...
        for i in self.i:
            self.wg.append(
                len(self) /
                self.group_sizes[self.nb_groups * self.y[i] + self.g[i]])
    ...
    def subsample_(self, subsample_what):
        ...
        for p in perm:
            ...
            if (subsample_what == "groups"
                and counts_g[self.nb_groups * int(y) + int(g)] < min_size) or\
               (subsample_what == "classes" and counts_y[int(y)] < min_size):
...
class Waterbirds(GroupDataset):
    def __init__(self, split, subsample_what=None, duplicates=None):
        root = os.path.join(
            os.environ['DATASETS_PATH'],
            "waterbirds/waterbird_complete95_forest2water2/")
        metadata = "metadata/metadata_waterbirds.csv"
        ...
        super().__init__(
            split, root, metadata, transform, subsample_what, duplicates)
...
class CelebA(GroupDataset):
    def __init__(self, split, subsample_what=None, duplicates=None):
        ...
        metadata = "metadata/metadata_celeba.csv"
...
class MultiNLI(GroupDataset):
    def __init__(self, split, subsample_what=None, duplicates=None):
        root = os.path.join(
            os.environ['DATASETS_PATH'], "multinli", "glue_data", "MNLI")
        metadata = "metadata/metadata_multinli.csv"
...
class CivilComments(GroupDataset):
    def __init__(self, split, subsample_what=None, duplicates=None,
                 granularity="coarse"):
        metadata = "metadata/metadata_civilcomments_{}.csv".format(granularity)

        text = pd.read_csv(
            os.path.join(
                os.environ['DATASETS_PATH'],
                "civilcomments/civilcomments_{}.csv".format(granularity)))
...
class CivilCommentsFine(CivilComments):
    def __init__(self, split, subsample_what=None, duplicates=None):
        super().__init__(split, subsample_what, duplicates, "fine")
...
class Toy(GroupDataset):
    ...
    def make_dataset(self, n_samples=1000, dim_noise=1200, blob_std=0.15,
                     core_cor=1.0, spu_cor=0.8, train=True):
        X = make_blobs(
            n_samples=n_samples, centers=1, cluster_std=[blob_std]
        )[0]
...
def get_loaders(dataset_name, batch_size, method="erm", duplicates=None):
    Dataset = {
        "waterbirds": Waterbirds,
        "celeba": CelebA,
        "multinli": MultiNLI,
        "civilcomments":
        CivilCommentsFine if method in ("subg", "rwg") else CivilComments,
        "toy": Toy,
    }[dataset_name]
    ...
    def dl(dataset, bs, shuffle, weights):
        if weights is not None:
            ...
            sampler = None
        return DataLoader(
            dataset, batch_size=bs, shuffle=shuffle, sampler=sampler,
    ...
    dataset_tr = Dataset("tr", subsample_what, duplicates)
    ...
    return {
        ...
        "va": dl(Dataset("va", None), 128, False, None),
        "te": dl(Dataset("te", None), 128, False, None)
    }
